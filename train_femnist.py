    ...
    with ctx:
            ...
            for i in range(args.nbyz):
                ...
                for epoch in range(5):
                    with autograd.record():
                        #minibatch = np.random.choice(range(each_worker_data[i].shape[0]), size=8,
                                                     #replace=False)
                        output1 = net1(each_worker_data[i][:])
                        loss1 = softmax_cross_entropy(output1, each_worker_label[i][:]) * alpha
                        for j, (param) in enumerate(net1.collect_params().values()):
        ...
        for e in range(epochs):
            ...
            if args.advanced_backdoor:
                param_list[:args.nbyz] = train_malicious_net(weight.detach().copy(), param_list, lr)
